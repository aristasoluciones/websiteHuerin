<div class="section spacing-default tersus-block-services-9" id="auditoria">
    <div class="background tersus-about-custom audit"></div>
    <div class="bind-narrow bind-narrow-md">
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <div class="icon-box icon-box-boxed2-huerin">
                        <h4>AUDITORIA</h4>
                        <p>Contamos con los profesionales idoneos para analizar los resultados del registro de operaciones efectuados. con el fin de comprobar que las disposiciones
                            hayan sido cumplidas de acuerdo a las normas.</p>
                        <h5>Servicios</h5>
                        <ul class="list-group">
                            <li><a class="list-body" id="auditoria-1"><i class="fa fa-circle huerin-orange"></i>Estudios de precios de transferencia.</a>
                                <div class="circle-detail popup-detail" id="auditoria-1">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Estudio de precios de transferencia</h4>
                                        <p class="body-circle-detail">Este es un análisis de las operaciones entre compañias hermanas para determinar los correctos valores de mercado.</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-2"><i class="fa fa-circle huerin-orange"></i>Dictamen de enajenacion de acciones.</a>
                                <div class="circle-detail popup-detail" id="auditoria-2">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Dictamen de enajenacion de acciones</h4>
                                        <p class="body-circle-detail">Este es un servicio para las personas físicas que venden acciones donde delimita el valor correcto de las acciones además del correcto calculo de impuestos en dicha venta.</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-3"><i class="fa fa-circle huerin-orange"></i>Dictamen fiscal SIPRED</a>
                                <div class="circle-detail popup-detail" id="auditoria-3">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Dictamen fiscal SIPRED</h4>
                                        <p class="body-circle-detail">Este es un servicio que incluye el análisis de la contabilidad, emisión de una opinión en cuanto al correcto cumplimiento de las normas contables y leyes fiscales, además del llenado de la declaración correspondiente.</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-4"><i class="fa fa-circle huerin-orange"></i>Dictamen financiero.</a>
                                <div class="circle-detail popup-detail" id="auditoria-4">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Dictamen financiero</h4>
                                        <p class="body-circle-detail">Este es un servicio que incluye el análisis de la contabilidad, emisión de una opinión en cuanto al correcto cumplimiento de las normas contables y leyes fiscales.</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-5"><i class="fa fa-circle huerin-orange"></i>Auditoria 32H.</a>
                                <div class="circle-detail popup-detail" id="auditoria-5">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Auditoria 32H</h4>
                                        <p class="body-circle-detail">Te ayudamos a presentar la declaración informativa sobre operaciones con el extranjero regulada en el articulo 32H del código fiscal de la federación.</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-6"><i class="fa fa-circle huerin-orange"></i>Due Diligence.</a>
                                <div class="circle-detail popup-detail" id="auditoria-6">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Due Diligence</h4>
                                        <p class="body-circle-detail">Este es un servicio que incluye el análisis y valuación de la sociedad, determinando los valores de la misma para su compra venta.</p>
                                    </div>
                                </div>
                            </li>
                        </ul>
                        <h5>Trámites y Gestoría</h5>
                        <ul class="list-group">
                            <li><a class="list-body" id="auditoria-7"><i class="fa fa-circle huerin-orange"></i>Cambio de domicilio.</a>
                                <div class="circle-detail popup-detail" id="auditoria-7">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Cambio de domicilio</h4>
                                        <p class="body-circle-detail">Apertura o cierre de establecimiento o sucursales.</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-8"><i class="fa fa-circle huerin-orange"></i>Tramite de las claves obligatorias de las diferentes autoridades.</a>
                                <div class="circle-detail popup-detail" id="auditoria-8">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Tramite de las claves obligatorias de las diferentes autoridades</h4>
                                        <p class="body-circle-detail">Visite el area de contacto para mas información</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-9"><i class="fa fa-circle huerin-orange"></i>Aumento o disminución de obligaciones.</a>
                                <div class="circle-detail popup-detail" id="auditoria-9">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Aumento o disminución de obligaciones</h4>
                                        <p class="body-circle-detail">Visite el area de contacto para mas información</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-10"><i class="fa fa-circle huerin-orange"></i>Organización y archivo de comprobantes y declaraciones.</a>
                                <div class="circle-detail popup-detail" id="auditoria-10">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Organización y archivo de comprobantes y declaraciones</h4>
                                        <p class="body-circle-detail">Visite el area de contacto para mas información</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-11"><i class="fa fa-circle huerin-orange"></i>Revisión a requerimientos de las diferentes autoridades.</a>
                                <div class="circle-detail popup-detail" id="auditoria-11">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Revisión a requerimientos de las diferentes autoridades</h4>
                                        <p class="body-circle-detail">Visite el area de contacto para mas información</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-12"><i class="fa fa-circle huerin-orange"></i>Revisión a auditorias de las diferentes autoridades.</a>
                                <div class="circle-detail popup-detail" id="auditoria-12">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Revisión a auditorias de las diferentes autoridades</h4>
                                        <p class="body-circle-detail">Visite el area de contacto para mas información</p>
                                    </div>
                                </div>
                            </li>
                            <li><a class="list-body" id="auditoria-13"><i class="fa fa-circle huerin-orange"></i>Contestación a consultas relativas a la operación de la empresa en el área contable fiscal o financiera.</a>
                                <div class="circle-detail popup-detail" id="auditoria-13">
                                    <div class="content-circle">
                                        <h4 class="title-circle-detail">Contestación a consultas relativas a la operación de la empresa en el área contable fiscal o financiera</h4>
                                        <p class="body-circle-detail">Visite el area de contacto para mas información</p>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>